...
def intersectAngle(x1,y1,x2,y2,x3,y3,x4,y4):
    """calculates the angle of two intersecting lines that are span between P1, P2 and P3, P4
    ...
    Args:
        ...
        y3: y-coordinate of P3
        x4: x-coordinate of P4
        y4: y-coordinate of P4

    Returns:
        The angle of the intersecting lines in degrees
    """
    try:
        m1 = (y2-y1)/(x2-x1)
        m2 = (y4-y3)/(x4-x3)
        ...
        return 0
    ang = atan((m2-m1)/(1+m1*m2))
    return degrees(ang)

def fingerRaised(frame,landmarks, visualize = False):
    """Checks if the index finger is raised

    Args:
        frame: the image to check
        landmarks: The hand landmarks generated by mediapipe
        visualize (bool, optional): If set to True, this function displays the calculated angle to understand whats happening. Defaults to False.

    Returns:
        True if the index finger is raised, False otherwise
    """
    h, w, c = frame.shape
    if landmarks is not None:
        #calculate the angle and round to full int
        ang = round(intersectAngle(int(landmarks[5].x * w), int(landmarks[5].y * h), int(landmarks[17].x * w), int(landmarks[17].y * h),int(landmarks[8].x * w), int(landmarks[8].y * h), int(landmarks[20].x * w), int(landmarks[20].y * h)))
        if(visualize):
            cv2.putText(frame,str(ang), (10,280), cv2.FONT_HERSHEY_PLAIN, 3, (0,0,255), 3)
        if abs(ang) > 30: #threshold of 30 degrees is choosen experimentally
            return True
        else:
            return False
...
def curState(frame,landmarks, side, visualize = False):
    ...
    if landmarks is None:
        ...
        state = "STOP"
    elif fingerRaised(frame, landmarks, visualize):
        state = "COME HERE"
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        h, w, c = frame.shape
        landmarks, side = findHand(frame, handDetector, True) #let mediapipe detect the hand
        
        cv2.putText(frame,curState(frame,landmarks,side,False), (10,140), cv2.FONT_HERSHEY_PLAIN, 3, (0,0,255), 3) #check if fingers are closed and write the result onto the wrist
